...
cpdef (double, int, double, int) c_pertub_error(
    ...
    cdef:
        pertub_res optimum = min_pertub_error(
            ...
            &bs_prob[0,0], ii, a_bs, prob_thresh,
            x_rows, x_cols, percent, n_jobs)
        double optimum_error = optimum.error
        int optimum_k = optimum.k
        double optimum_step = optimum.step
        int optimum_coord_ind = optimum.coord_ind

    return optimum_error, optimum_k, optimum_step, optimum_coord_ind